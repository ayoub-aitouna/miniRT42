...
t_vector *get_color(t_txtr_img *txtr_img, t_uv_cords cords)
{
	...
	u = (txtr_img->img_width - 1) - (floor((cords.u + 1) * ((txtr_img->img_width - 1) / 2.f)));
	v = (txtr_img->img_height - 1) - (floor((double)(cords.v + 1) * ((txtr_img->img_height - 1) / 2.f)));
	mapping_cords = (t_uv_cords){.u = u, .v = v};
	dst = txtr_img->img->addr + ((int)mapping_cords.v * txtr_img->img->line_length + (int)mapping_cords.u * (txtr_img->img->bits_per_pixel / 8));
