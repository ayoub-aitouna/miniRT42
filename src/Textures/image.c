...
/*   Updated: 2023/07/09 00:31:18 by aaitouna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
vector_t *get_color(t_textures *this, t_uv_cords cords)
{
	...
	mapping_cords = (t_uv_cords){
		.u = (this->img_width - 1) - (floor((cords.u + 1) * ((this->img_width - 1) / 2.f))),
		.v = (this->img_height - 1) - (floor((double)(cords.v + 1) * ((this->img_height - 1) / 2.f)))};
	dst = this->img->addr + ((int)mapping_cords.v * this->img->line_length + (int)mapping_cords.u * (this->img->bits_per_pixel / 8));
