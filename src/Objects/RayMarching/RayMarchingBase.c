...
/*   Updated: 2023/07/16 19:36:27 by aaitouna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_vector *dir_v, t_vector *params)
{
	...
	l_normal = ms_minus(cur_location, num_muliplication(dir_v, 0.01), 1);
	normal = (t_vector){calculat_sdf_diff(this, l_normal, (t_vector){D_S, 0.0,
			0.0}, *params), calculat_sdf_diff(this, l_normal, (t_vector){0.0,
			D_S, 0.0}, *params), calculat_sdf_diff(this, l_normal,
			(t_vector){0.0, 0.0, D_S}, *params)};
	normalize(&normal);
	m_normal = get_norm(this, &normal);
	result = *m_normal;
	free_list((void *[]){l_normal, m_normal}, 2);
	return (result);
}
