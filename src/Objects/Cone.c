...
void cone_cap_intersection(vector_t p, vector_t n, cep_t propretries)
{
	...
	propretries.intersections[2] = addition(&p, num_muliplication(&n, propretries.t[2]));

	if (propretries.t[2] > 0.0 &&
		(sqrt(pow(propretries.intersections[2]->x, 2) + pow(propretries.intersections[2]->y, 2))) < 1.0)
...
void calulcat_cone_intersection(vector_t p, vector_t n, cep_t propretries)
{
	...
	if (numsqrt > 0)
	{
		...
		propretries.intersections[0] = addition(&p, num_muliplication(&n, propretries.t[0]));
		propretries.intersections[1] = addition(&p, num_muliplication(&n, propretries.t[1]));
		if (propretries.t[0] > 0 && in_range(propretries.intersections[0]->z, .0, 1.))
			propretries.valide_intersections[0] = TRUE;
		else
			propretries.t[0] = MAX_V;
		if (propretries.t[1] > 0 && in_range(propretries.intersections[1]->z, .0, 1.))
				  ...
				  vector_t *local_normal, vector_t *local_color)
{
	...
	calulcat_cone_intersection(p, n, (cep_t){.t = t, .intersections = intersections, .valide_intersections = valide_intersections});
	if (!close_enough(n.z, 0.0f))
		cone_cap_intersection(p, n, (cep_t){.t = t, .intersections = intersections, .valide_intersections = valide_intersections});
	delete_ray(bck_ray);
