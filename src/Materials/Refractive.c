...
/*   Updated: 2023/07/08 23:06:43 by aaitouna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ray_t *reflacted_ray(ray_t *m_ray, double r, propretries_t *prop)
{
	...
	refractive_vector =
		ms_addition(num_muliplication(ray_norm, r), num_muliplication(tmp_normal, f), 2);
	free_list((void *[]){ray_norm, tmp_normal}, 2);
	return ray(ms_addition(&prop->int_point, num_muliplication(refractive_vector, 0.01), 1),
			   ms_addition(&prop->int_point, refractive_vector, 1));
}
